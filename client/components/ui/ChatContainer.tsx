...
useEffect(() => {
  const handler = (payload: any) => {
    ...
    // cache nội dung mới để UI update ngay mà không sửa props
    setEditedCache((prev) => ({ ...prev, [messageId]: content }));

    // nếu đang edit đúng message đó thì đóng input
  ...
  <div className="flex items-center justify-between bg-yellow-500/10 px-4 py-2 text-sm text-yellow-300 border-b border-yellow-500/30">
    <div className="flex items-center gap-2 truncate">
      ...
      <span className="text-xs text-yellow-400 opacity-80">
        – {pinnedMessage.pinnedBy === userId ? "Bạn" : pinnedMessage.pinnedBy} ghim
      </span>
    </div>
...
>
  ...
  >
          <div className="font-semibold text-blue-400 mb-0.5">
            ↩️ {msg.replyTo.userId === userId ? "Bạn" : msg.replyTo.userId}
          </div>
          ...
          {msg.replyTo.images?.length ? (
            <div className="italic text-gray-300">📷 Ảnh</div>
          ) : msg.replyTo.fileUrl ? (
            <div className="italic text-gray-300">📎 File</div>
          ) : null}
        </div>
      )}
